...
1. Run `yarn translation:extract` after adding or changing UI text. Review the
   ...
   intentional whitespace. `i18next-cli` resolves aliased `t` functions through
   scope-aware analysis, but dynamic keys must be added by hand;
   `removeUnusedKeys: false` in `i18next.config.ts` preserves them.
...
The app uses the i18next JSON v4 plural format, resolved through
`Intl.PluralRules`. Hermes does not implement that API, so `i18n.js` imports the
`intl-pluralrules` polyfill before initialising i18next; without it only
`_one`/`_other` would resolve and Russian, Japanese and Chinese would break on
device while passing in Jest. For each family supply the language's native
suffixes:

| Languages | Required keys |
|---|---|
| en, de, nl | `_one` (1), `_other` (rest) |
| es, fr, pt | `_one` (1), `_other` (rest), `_many` (millions) |
| ru | `_one` (1, 21, 101), `_few` (2–4, 22), `_many` (0, 5, 11–14), `_other` (decimals) |
| ja, zh | `_other` |

A key without a suffix is optional; i18next resolves plurals by suffix only.
