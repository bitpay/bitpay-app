  ...
  return (
    <TabBarContainer darkMode={dark} totalWidth={totalWidth}>
      {state.routes.map((route, index) => {
        ...
        const tabBarLabel = typeof options.tabBarLabel === 'string' ? options.tabBarLabel : route.name;
        const tabBarIcon = options?.tabBarIcon ? options.tabBarIcon({ focused: isFocused, color: White, size: 20 }) : null;
        return (
          <TabButton
            key={route.key}
            isFocused={isFocused}
