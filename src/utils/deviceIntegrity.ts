...
// Cached for cheap reads (startup telemetry, useDeviceIntegrity). Sensitive-action
// checks force a refresh: a Frida-style hook can attach after startup.
...
export const getDeviceIntegrity = (
  ...
  try {
    ...
    // Not cached: a transient native failure must not mark the device clean
...
export const assertDeviceIntegrityForSensitiveAction = (
  ...
  const result = getDeviceIntegrity(true);
