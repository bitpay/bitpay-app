...
export const validatePin = (
  ...
  if (COMMON_WEAK_PINS.includes(pin)) {
    errors.push('Please choose a more unique one');
  }
...
/**
 * Generate a cryptographically secure salt for PIN hashing
 * Uses the same approach as generateSalt but with more entropy
 *
 * @returns Hex-encoded salt string
 */
const generatePinSalt = (): string => {
  // Use multiple random sources for better entropy
  const randomSource1 = Math.random().toString() + Date.now().toString();
  const randomSource2 = Math.random().toString() + performance.now().toString();

  const salt = Pbkdf2.pbkdf2Sync(
    randomSource1,
    randomSource2,
    PIN_CONFIG.SALT_INTERACTIONS,
    PIN_CONFIG.SALT_LENGTH,
    'sha256',
  ).toString('hex');
...
export const verifyAndMigratePin = (
  ...
  if (storedSalt) {
    ...
    const isValid =
      computedHash === storedHash && timingSafeEqual(computedHash, storedHash);
