...
export const createWalletsForAccounts = async (
  ...
  const results = await Promise.all(
    accountsArray.map(async account => {
      ...
      logManager.info(
        `[PERF IMPORT OPTIMIZATION] createWalletsForAccounts.account | ${JSON.stringify(
          {
            account,
            requestedCurrencies: currencies.length,
            missingCurrencies: missingCurrencies.length,
            skippedCurrencies: currencies.length - missingCurrencies.length,
          },
        )}`,
      );
