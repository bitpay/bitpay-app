...
describe('deviceIntegrity', () => {
  ...
  describe('getDeviceIntegrity', () => {
    ...
    it('does not cache a fail-open result', () => {
      mockIsJailBroken.mockImplementationOnce(() => {
      ...
      mockIsJailBroken.mockReturnValue(true);
      expect(getDeviceIntegrity().isCompromised).toBe(true);
    });
  ...
  describe('assertDeviceIntegrityForSensitiveAction', () => {
    ...
    it('bypasses the cache so a hook attached after startup is caught', () => {
      const prevDev = (global as any).__DEV__;
      (global as any).__DEV__ = false;
