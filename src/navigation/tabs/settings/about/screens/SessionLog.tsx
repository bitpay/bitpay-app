...
const SessionLogs = ({}: SessionLogsScreenProps) => {
  ...
  const previousSessionLogs = useMemo(
    () =>
      persistedLogs.filter(
        log => new Date(log.timestamp).getTime() < currentSessionStartTime,
      ),
    [currentSessionStartTime, persistedLogs],
  );

  const filteredPersistedLogs = useMemo(
    () => previousSessionLogs.filter(log => log.level <= filterLevel),
    [filterLevel, previousSessionLogs],
  );
  ...
  const showDisclaimer = useCallback(
    (option: 'email' | 'share') => {
      ...
      const persistedLogString = previousSessionLogs.length
        ? 'Previous Sessions\n\n' +
          printLogs(previousSessionLogs) +
          '\n\nCurrent Session\n\n'
        : '';
    ...
    [closeOptions, handleEmail, logs, previousSessionLogs, shareFile, t],
  );
