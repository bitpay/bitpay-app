...
const InviteCosigners: React.FC<Props> = ({navigation, route}) => {
  ...
  useEffect(() => {
    if (isComplete && pendingJoinCode && currentStep === 2) {
      const timer = setTimeout(() => {
        setCurrentStep(3);
      }, 1500);
      return () => clearTimeout(timer);
    }
  }, [isComplete, pendingJoinCode, currentStep]);
  ...
  const renderModalTopSection = () => {
    ...
    if (currentStep === 3 && pendingJoinCode) {
      return (
        <TopSection>
          <QRSectionContainer>
            <QRContainer>
    ...
    return (
      <StepsSection>
        <StepsContainer>
          ...
          <StepConnector completed={currentStep > 1 || isComplete} />

          <StepRow>
            <StepIndicator
              active={currentStep === 2 && !isComplete}
