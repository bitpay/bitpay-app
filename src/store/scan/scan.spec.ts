...
describe('incomingData', () => {
  ...
  it('Should handle ETH plain Address', async () => {
    ...
    for (let i = 0; i < 1; i++) {
      expect(loggerSpy).toHaveBeenNthCalledWith(
        i + 1,
        '[scan] Incoming-data: eth plain address',
      );
      expect(navigationSpy).toHaveBeenNthCalledWith(i + 1, 'Wallet', {
        params: {
          context: 'scanner',
  ...
  it('Should handle MATIC plain Address', async () => {
    ...
    for (let i = 0; i < 1; i++) {
      expect(loggerSpy).toHaveBeenNthCalledWith(
        i + 1,
        '[scan] Incoming-data: eth plain address', // yes eth... for now
      );
      expect(navigationSpy).toHaveBeenNthCalledWith(i + 1, 'Wallet', {
        params: {
          context: 'scanner',
          recipient: {
            address: data[i],
            chain: 'eth', // yes eth... for now
            currency: 'eth', // yes eth... for now
            destinationTag: undefined,
            email: undefined,
            name: undefined,
            network: undefined, // for showing testnet and livenet wallets
  ...
  it('Should handle ETH URI as address if there is no amount', async () => {
  ...
  it('Should handle XRP URI as address if there is no amount', async () => {
