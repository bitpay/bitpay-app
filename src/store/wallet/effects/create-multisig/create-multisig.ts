  ...
  async (dispatch, getState): Promise<Key> => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const walletFromBWS = await new Promise<any>(
          (resolveWallet, rejectWallet) => {
            tssKeyGen
              .on('roundready', (r: number) => {
                ...
                try {
                  ...
                  // Session might be complete, can't export
                }
              })
        ...
        const tssCredentials = _tssKey.createCredentials(null, {
          chain: chain.toUpperCase(),
          network,
          account: 0,
        });
        ...
        // TODO
        // Check if it's already a string (just in case) or an object
        if (typeof tssCredentials === 'string') {
          finalWalletClient.fromString(tssCredentials);
        } else {
          // It's a Credentials object - use fromObj or convert to string
          finalWalletClient.fromObj(tssCredentials);
        }

        finalWalletClient.credentials.walletId = walletFromBWS.id;

        await new Promise<void>((resolveOpen, rejectOpen) => {
          finalWalletClient.openWallet({}, (err: Error, status: any) => {
            if (err) {
              logManager.error(
                `[TSS Ceremony] Error opening wallet: ${err.message}`,
              );
              rejectOpen(err);
            } else {
              logManager.debug(
                `[TSS Ceremony] Wallet opened, sharedEncryptingKey: ${!!finalWalletClient
                  .credentials.sharedEncryptingKey}`,
              );
              resolveOpen();
            }
          });
        ...
        finalWalletClient.credentials.addWalletPrivateKey(walletPrivKey);
        logManager.debug(
          `[TSS] Generated walletPrivKey, sharedEncryptingKey: ${!!finalWalletClient
            .credentials.sharedEncryptingKey}`,
        );
  ...
  async (dispatch, getState): Promise<Key> => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const BWCKey = BWC.getKey();
        const copayerName = opts.copayerName || 'Co-signer';
        ...
        await new Promise<void>((resolveOpen, rejectOpen) => {
          finalWalletClient.openWallet({}, (err: Error, status: any) => {
            if (err) {
              logManager.error(
                `[TSS Join] Error opening wallet: ${err.message}`,
              );
              rejectOpen(err);
            } else {
              logManager.debug(
                `[TSS Join] Wallet opened, sharedEncryptingKey: ${!!finalWalletClient
                  .credentials.sharedEncryptingKey}`,
              );
              resolveOpen();
            }
        ...
        const finalKey = buildTssKeyObj({
          tssKey: _tssKey,
          wallets: [finalWallet],
          keyName: walletFromBWS.name || `TSS Wallet`,
        });
