...
describe('bindWalletKeys inbound — wallet secrets', () => {
  ...
  it('keeps the secrets until the encrypted slice reached disk', () => {
    const persisted: any = bindWalletKeys.in!(makeState(false), 'WALLET', {});

    expect(persisted.keys.key1.properties).toBeDefined();
    expect(persisted.keys.key1.wallets[0].credentials.requestPrivKey).toBe(
      'request-priv-key',
    );
  });
  ...
  it('drops them once the migration completed', () => {
    ...
    expect(wallet.credentials.requestPrivKey).toBeUndefined();
    expect(wallet.credentials.walletPrivKey).toBeUndefined();
    expect(wallet.credentials.xPubKey).toBe('xpub-public');
  });

  it('does not strip the secrets off the in-memory state', () => {
    const state = makeState(true);
    bindWalletKeys.in!(state, 'WALLET', {});

    expect(state.keys.key1.properties).toBeDefined();
    expect(state.keys.key1.wallets[0].credentials.requestPrivKey).toBe(
      'request-priv-key',
    );
  });
});
